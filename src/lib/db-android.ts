...
async function runMigrations(db: any) {
  ...
  for (const sql of alterMigrations) {
    try { await db.run({ statement: sql, values: [] }) } catch { /* already exists */ }
  }
}
...
export async function getBinderById(id: string): Promise<(BinderRow & { pages: PageRow[]; cards: CardRow[]; totalValue: number; cardCount: number }) | null> {
  const db = await getDb()
  const binderRes = await db.query({ statement: 'SELECT * FROM binders WHERE id = ?', values: [id] })
  ...
  const cardsRes = await db.query({
    statement: 'SELECT * FROM binder_cards WHERE binderId = ? ORDER BY createdAt ASC',
    values: [id],
  })
...
export async function createBinder(data: { name: string; description?: string; coverColor?: string | null; coverImagePath?: string | null; coverPattern?: string | null; coverPreset?: string | null }): Promise<BinderRow> {
  ...
  const ts = now()
  await db.run({
    statement: 'INSERT INTO binders (id, name, description, coverColor, coverImagePath, coverPattern, coverPreset, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    values: [id, data.name, data.description || null, data.coverColor || null, data.coverImagePath || null, data.coverPattern || null, data.coverPreset || null, ts, ts],
  })
  const res = await db.query({ statement: 'SELECT * FROM binders WHERE id = ?', values: [id] })
  return res.values![0]
}
...
export async function updateBinder(id: string, updates: { name?: string; description?: string | null; coverColor?: string | null; coverImagePath?: string | null; coverPattern?: string | null; coverPreset?: string | null }): Promise<BinderRow> {
  ...
  params.push(id)
  await db.run({ statement: `UPDATE binders SET ${parts.join(', ')} WHERE id = ?`, values: params })
  const res = await db.query({ statement: 'SELECT * FROM binders WHERE id = ?', values: [id] })
  return res.values![0]
}
...
export async function deleteBinder(id: string): Promise<boolean> {
  const db = await getDb()
  await db.run({ statement: 'DELETE FROM binders WHERE id = ?', values: [id] })
  return true
}
...
export async function getPageById(pageId: string): Promise<(PageRow & { cards: CardRow[] }) | null> {
  const db = await getDb()
  const pageRes = await db.query({ statement: 'SELECT * FROM pages WHERE id = ?', values: [pageId] })
  const page = pageRes.values?.[0]
  if (!page) return null
  const cardsRes = await db.query({
    statement: 'SELECT * FROM binder_cards WHERE pageId = ? ORDER BY COALESCE(position, 9999) ASC, createdAt ASC',
    values: [pageId],
  })
...
export async function createPage(data: { binderId: string; name?: string; imagePath?: string; status?: string; cols?: number; rows?: number }): Promise<PageRow> {
  ...
  const id = uid()
  const ts = now()
  const countRes = await db.query({ statement: 'SELECT COUNT(*) as c FROM pages WHERE binderId = ?', values: [data.binderId] })
  const count = countRes.values?.[0]?.c ?? 0
  const pageNumber = count + 1
  await db.run({
    statement: 'INSERT INTO pages (id, binderId, pageNumber, position, name, imagePath, status, cols, rows, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    values: [id, data.binderId, pageNumber, pageNumber, data.name || `Page ${pageNumber}`, data.imagePath || '', data.status || 'pending', data.cols || 3, data.rows || 3, ts],
  })
  const res = await db.query({ statement: 'SELECT * FROM pages WHERE id = ?', values: [id] })
  return res.values![0]
}
...
export async function updatePage(id: string, data: { name?: string; position?: number; status?: string; rawAiOutput?: string; processedAt?: string; cols?: number; rows?: number; imagePath?: string }): Promise<PageRow> {
  ...
  await db.run({
    statement: `
      ...
      WHERE id = ?
    `,
    values: [data.name ?? null, data.position ?? null, data.status ?? null, data.rawAiOutput ?? null, data.processedAt ?? null, data.cols ?? null, data.rows ?? null, data.imagePath ?? null, id],
  })
  const res = await db.query({ statement: 'SELECT * FROM pages WHERE id = ?', values: [id] })
  return res.values![0]
}
...
export async function deletePage(id: string): Promise<boolean> {
  const db = await getDb()
  await db.run({ statement: 'DELETE FROM binder_cards WHERE pageId = ?', values: [id] })
  await db.run({ statement: 'DELETE FROM pages WHERE id = ?', values: [id] })
  return true
}
...
export async function reorderPages(binderId: string, orderedIds: string[]): Promise<boolean> {
  ...
  for (let i = 0; i < orderedIds.length; i++) {
    await db.run({ statement: 'UPDATE pages SET position = ? WHERE id = ? AND binderId = ?', values: [i, orderedIds[i], binderId] })
  }
  return true
}
...
export async function getCards(binderId: string, pageId?: string): Promise<CardRow[]> {
  ...
  if (pageId) {
    const res = await db.query({
      statement: 'SELECT * FROM binder_cards WHERE binderId = ? AND pageId = ? ORDER BY COALESCE(position, 9999) ASC, createdAt ASC',
      values: [binderId, pageId],
    })
    return res.values ?? []
  }
  const res = await db.query({
...
export async function createCard(data: Partial<CardRow>): Promise<CardRow> {
  ...
  if (data.pageId && position === null) {
    const res = await db.query({ statement: 'SELECT COUNT(*) as c FROM binder_cards WHERE pageId = ?', values: [data.pageId] })
  ...
  const res = await db.query({ statement: 'SELECT * FROM binder_cards WHERE id = ?', values: [id] })
  return res.values![0]
}
...
export async function updateCard(id: string, data: Partial<CardRow> & { tradeList?: boolean }): Promise<CardRow> {
  ...
  await db.run({ statement: `UPDATE binder_cards SET ${parts.join(', ')} WHERE id = ?`, values: params })
  const res = await db.query({ statement: 'SELECT * FROM binder_cards WHERE id = ?', values: [id] })
  return res.values![0]
}
...
async function updateCardCondition(id: string, condition: string | null): Promise<CardRow> {
  const db = await getDb()
  const cardRes = await db.query({ statement: 'SELECT priceBase, priceMarket, isCustom FROM binder_cards WHERE id = ?', values: [id] })
  const card = cardRes.values?.[0]
  if (!card) throw new Error('Card not found')

  if (card.isCustom) {
    await db.run({ statement: 'UPDATE binder_cards SET condition = ?, updatedAt = ? WHERE id = ?', values: [condition ?? null, now(), id] })
  } else {
    ...
    await db.run({
      statement: 'UPDATE binder_cards SET condition = ?, priceMarket = ?, priceBase = COALESCE(priceBase, ?), updatedAt = ? WHERE id = ?',
      values: [condition ?? null, newPrice, base ?? null, now(), id],
    })
  }
...
export async function deleteCard(id: string): Promise<boolean> {
  const db = await getDb()
  await db.run({ statement: 'DELETE FROM binder_cards WHERE id = ?', values: [id] })
  return true
}
...
export async function reorderPageCards(pageId: string, positions: Array<{ id: string; position: number }>): Promise<boolean> {
  ...
  for (const { id, position } of positions) {
    await db.run({ statement: 'UPDATE binder_cards SET position = ?, updatedAt = ? WHERE id = ? AND pageId = ?', values: [position, ts, id, pageId] })
  }
  return true
}
...
export async function moveCardsToPage(cardIds: string[], targetPageId: string): Promise<boolean> {
  ...
  for (let i = 0; i < cardIds.length; i++) {
    await db.run({ statement: 'UPDATE binder_cards SET pageId = ?, position = ?, updatedAt = ? WHERE id = ?', values: [targetPageId, i, ts, cardIds[i]] })
  }
  return true
}
...
export async function updateCardPricesFull(id: string, pricing: any, condition: string | null, eurUsdRate: number): Promise<void> {
  ...
  await db.run({
    statement: 'UPDATE binder_cards SET priceLow = ?, priceMid = ?, priceMarket = ?, priceHigh = ?, priceUpdatedAt = ?, updatedAt = ?, priceBase = ? WHERE id = ?',
    values: [priceLow, null, adjustedMarket, null, new Date().toISOString(), now(), rawMarket, id],
  })
}
...
export async function getCardsForRefresh(binderId: string): Promise<CardRow[]> {
  const db = await getDb()
  const res = await db.query({
    statement: `SELECT * FROM binder_cards WHERE binderId = ? AND tcgApiId NOT LIKE 'unmatched-%' AND isCustom = 0 AND cardGame = 'pokemon'`,
    values: [binderId],
  })
  return res.values ?? []
}
...
export async function getOpCardsForRefresh(binderId: string): Promise<CardRow[]> {
  const db = await getDb()
  const res = await db.query({
    statement: `SELECT * FROM binder_cards WHERE binderId = ? AND isCustom = 0 AND cardGame = 'onepiece'`,
    values: [binderId],
  })
  return res.values ?? []
}
...
export async function updateOpCardPrice(id: string, priceMarket: number | null, priceLow: number | null): Promise<void> {
  const db = await getDb()
  await db.run({
    statement: 'UPDATE binder_cards SET priceMarket = ?, priceLow = ?, priceBase = ?, priceUpdatedAt = ?, updatedAt = ? WHERE id = ?',
    values: [priceMarket, priceLow, priceMarket, new Date().toISOString(), now(), id],
  })
}
...
export async function getSlabs(): Promise<SlabRow[]> {
  const db = await getDb()
  const res = await db.query({ statement: 'SELECT * FROM slabs ORDER BY createdAt DESC', values: [] })
  return res.values ?? []
}
...
export async function createSlab(data: { name: string; gradingCompany: string; grade: string; certNumber?: string | null; pricePaid?: number | null; currentPrice?: number | null; imageUrl?: string | null }): Promise<SlabRow> {
  ...
  const ts = now()
  await db.run({
    statement: 'INSERT INTO slabs (id, name, gradingCompany, grade, certNumber, pricePaid, currentPrice, imageUrl, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    values: [id, data.name, data.gradingCompany || '', data.grade || '', data.certNumber ?? null, data.pricePaid ?? null, data.currentPrice ?? null, data.imageUrl ?? null, ts, ts],
  })
  const res = await db.query({ statement: 'SELECT * FROM slabs WHERE id = ?', values: [id] })
  return res.values![0]
}
...
export async function updateSlab(id: string, data: Partial<SlabRow>): Promise<SlabRow> {
  ...
  params.push(id)
  await db.run({ statement: `UPDATE slabs SET ${parts.join(', ')} WHERE id = ?`, values: params })
  const res = await db.query({ statement: 'SELECT * FROM slabs WHERE id = ?', values: [id] })
  return res.values![0]
}
...
export async function deleteSlab(id: string): Promise<boolean> {
  const db = await getDb()
  await db.run({ statement: 'DELETE FROM slabs WHERE id = ?', values: [id] })
  return true
}
...
export async function getWishlistCards(): Promise<WishlistCard[]> {
  const db = await getDb()
  const res = await db.query({ statement: 'SELECT * FROM wishlist_cards ORDER BY createdAt DESC', values: [] })
  return res.values ?? []
}
...
export async function createWishlistCard(data: { tcgApiId: string; name: string; setId: string; setName: string; collectorNumber: string; imageUrl?: string | null; cardGame: string; priority?: string; notes?: string | null }): Promise<WishlistCard> {
  ...
  const ts = now()
  await db.run({
    statement: 'INSERT INTO wishlist_cards (id, tcgApiId, name, setId, setName, collectorNumber, imageUrl, cardGame, priority, notes, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    values: [id, data.tcgApiId, data.name, data.setId || '', data.setName || '', data.collectorNumber || '', data.imageUrl ?? null, data.cardGame || 'pokemon', data.priority || 'medium', data.notes ?? null, ts],
  })
  const res = await db.query({ statement: 'SELECT * FROM wishlist_cards WHERE id = ?', values: [id] })
  return res.values![0]
}
...
export async function updateWishlistCard(id: string, data: { priority?: string; notes?: string | null }): Promise<WishlistCard> {
  ...
  if (parts.length > 0) {
    params.push(id)
    await db.run({ statement: `UPDATE wishlist_cards SET ${parts.join(', ')} WHERE id = ?`, values: params })
  }
  const res = await db.query({ statement: 'SELECT * FROM wishlist_cards WHERE id = ?', values: [id] })
  return res.values![0]
}
...
export async function deleteWishlistCard(id: string): Promise<boolean> {
  const db = await getDb()
  await db.run({ statement: 'DELETE FROM wishlist_cards WHERE id = ?', values: [id] })
  return true
}
